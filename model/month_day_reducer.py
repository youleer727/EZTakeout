...
def reducer():
    ...
    while (True):
        ...
        generate_new_file_name(cols[0]), (parse_start_time(cols[0]), int(cols[2]), float(cols[3]), float(cols[4]), calculate_time(cols[0], cols[1])))).reduceByKey(lambda a, b: appendAB(a, b))
        ...
        for i in range(0, dfMatrix.shape[0]):
            ...
            df.to_csv(path_or_buf=(datasets_path + 'processed/' + 'tripdata_reduced/' + row[0] + '.csv').encode('ascii', 'ignore'), index=False)
        month += 1
        if(month == 13):
            year = 2017
            month = 1
